...
class GateFusion(nn.Module):
    def __init__(self, dim: int):
        ...
        nn.init.normal_(self.proj.weight, meam=0.0, std=1e-3)
        nn.init.normal_(self.proj.bias, meam=0.0, std=1e-3)
...
class CogMemBank(nn.Module):
    def __init__(
        ...
        if self.use_timestep_pe:
            self.timestep_encoder = TimestepEmbedder(
                self.token_size, frequency_embedding_size=self.token_size // 4
            )
        else:
    ...
    def _consolidate_with_token_merge(self, episode_id):
        ...
        sims = []
        for i in range(T - 1):
            f1 = feats[i].flatten(1) if feats[i].dim() > 1 else feats[i].unsequeeze(0)
            f2 = (
                feats[i + 1].flatten(1)
                if feats[i + 1].dim() > 1
    ...
    def _memory_consolidate(
        self, episode_id, feat: torch.Tensor, timestep: Optional[torch.Tensor]
    ):
    ...
    def process_batch(
        ...
        for i in range(B):
            ...
            if len(hist) > 0:
                ...
                episode_mem = (
                    torch.stack(hist_feats, dim=0).reshape(-1, D).unsqueeze(0)
                )  # [1, T * N, D]
                ...
                retrieved_episode_mem = (
                    working_mem  # 没有历史信息, Fusion 结果是 Mem 本身.
                )

            # 3) memory gate fusion
